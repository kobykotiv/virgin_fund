...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (body.type !== undefined) update.strategy = body.type // Map type to strategy
    ...
    if (body.status !== undefined) {
      ...
      if (body.status === 'active') update.status = 'running'
      else if (body.status === 'paused') update.status = 'paused'
