...
export async function backtest(strategy, startDate, endDate) {
    try {
        // Fetch historical data for all assets
        const historicalData = await Promise.all(strategy.selectedAssets.map(asset => fetchHistoricalData(asset.symbol)));
...
function processDCAStrategy(strategy, historicalData, startDate, endDate) {
    const transactions = [];
...
function processMomentumStrategy(strategy, historicalData, startDate, endDate, 
// add arguments for the indicators
rsiPeriod = 14, macdFastPeriod = 12, macdSlowPeriod = 26, macdSignalPeriod = 9, rocPeriod = 10) {
...
function processTrendFollowingStrategy(strategy, historicalData, startDate, endDate, atrPeriod = 10, atrMultiplier = 3) {
    const transactions = [];
    let totalInvestment = 0;
    // Process each asset
    strategy.selectedAssets.forEach((asset, assetIndex) => {
...
function getIntervalInDays(frequency) {
    switch (frequency) {
        case 'daily': return 1;
...
function getHistoricalPrice(data, date) {
...
function getCurrentPrices(historicalData) {
    const prices = {};
