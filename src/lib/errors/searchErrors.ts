...
export class SearchError extends Error {
  ...
  constructor(details: Partial<ErrorDetails>) {
    ...
    this.details = {
      ...
      userMessage:
        details.userMessage || "Something went wrong. Please try again.",
      technical:
        details.technical || details.message || "Unknown technical details",
...
function handleApiError(error: AxiosError): SearchError {
  ...
  if (error.response?.status === 429) {
    return new SearchError({
      ...
      userMessage:
        "You've reached the maximum number of searches. Please wait a moment before trying again.",
  ...
  if (error.response?.status === 401) {
    return new SearchError({
      ...
      userMessage:
        "Unable to access financial data service. Please try again later.",
