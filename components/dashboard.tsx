...
export function Dashboard({ bots, apiConfig }: DashboardProps) {
  ...
  const [botsData, setBots] = useState<Bot[]>(bots) // Initialize botsData with the bots prop
  ...
  const getUniqueAssets = () => {
    ...
    const botAssets = botsData.flatMap((bot) => bot.assets || [])
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      // Fetch bots data
      const fetchedBotsData = await fetchBots()
      setBots(fetchedBotsData)
      ...
      if (assets.length > 0) {
        ...
        try {
          ...
          console.warn("Error fetching from primary market data source:", error)
  ...
  const pausedBots = botsData.filter((bot) => bot.status === "paused").length
  const errorBots = botsData.filter((bot) => bot.status === "error").length
  const totalBots = botsData.length

  // Calculate total profit/loss
  const totalPnL = botsData.reduce((sum, bot) => sum + (bot.performance?.totalPnL || 0), 0)
  const avgPnlPercentage =
    botsData.length > 0
      ? botsData.reduce((sum, bot) => sum + (bot.performance?.pnlPercentage || 0), 0) / botsData.length
      : 0

  // Calculate total trades
  const totalTrades = botsData.reduce((sum, bot) => sum + (bot.performance?.totalTrades || 0), 0)

  // Generate performance data for chart
  const performanceData = botsData.map((bot) => ({
    name: bot.name,
  ...
  const botTypeData = [
    { name: "Indicator", value: botsData.filter((bot) => bot.type === "indicator").length },
    { name: "Grid", value: botsData.filter((bot) => bot.type === "grid").length },
    { name: "DCA", value: botsData.filter((bot) => bot.type === "dca").length },
    { name: "Basket", value: botsData.filter((bot) => bot.type === "basket").length },
