...
export function generateAdvancedSignals(prices, volume, config = ADVANCED_STRATEGY) {
    ...
    const totalScore = signals.trend.score * config.indicators.trend.primary.weight +
...
function calculateTrendSignal(fastEMA, slowEMA, adx) {
...
function calculateMomentumSignal(rsi) {
    if (rsi > 70)
        return { signal: 'OVERBOUGHT', score: -0.8 };
    if (rsi < 30)
        return { signal: 'OVERSOLD', score: 0.8 };
    if (rsi > 60)
        return { signal: 'STRONG', score: 0.5 };
    if (rsi < 40)
        return { signal: 'WEAK', score: -0.5 };
    return { signal: 'NEUTRAL', score: 0 };
}
function calculateVolumeSignal(mfi) {
    if (mfi > 80)
        return { signal: 'OVERBOUGHT', score: -0.5 };
    if (mfi < 20)
        return { signal: 'OVERSOLD', score: 0.5 };
    return { signal: 'NEUTRAL', score: 0 };
}
