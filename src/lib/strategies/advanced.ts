...
export function generateAdvancedSignals(
  ...
  const fastEMA = calculateEMA(
    prices,
    config.indicators.trend.primary.params.fast,
  );
  const slowEMA = calculateEMA(
    prices,
    config.indicators.trend.primary.params.slow,
  );
  const adx = calculateADX(
    prices,
    config.indicators.trend.confirmation.params.period,
  );
  const rsi = calculateRSI(
    prices,
    config.indicators.momentum.primary.params.period,
  );
  const mfi = calculateMFI(
    prices,
    volume,
    config.indicators.volume.primary.params.period,
  );
...
function calculateMomentumSignal(rsi: number): {
  signal: string;
  score: number;
} {
